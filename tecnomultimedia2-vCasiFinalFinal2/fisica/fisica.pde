...
FMouseJoint golpea;
FCircle circulo;
FBox parlante; //este es el parlante
...
void setup() { 
  ...
  //efectoRadio = new PImage[4];
  PImage [] efectoRadio = new PImage [cantImg];
  ...
  for(int i = 0; i <efectoRadio.length; i++){
    efectoRadio[i] = loadImage("efectoRadio_0" + i + ".png" );
     efectoRadio[i].resize(200,200);
  }
  ...
  radio.resize(220,203);
  fondoEstado2 = loadImage("imagenes/fondo-jugando.png");
  //resize bailarines
  ...
  Fisica.init(this);
  mundo = new FWorld();
  parlante=new FBox(40, 10);
  meta=new FBox(400, 380);
  borde=new FBox(20, height+1000);
  bordeA= new FBox(10000, 10);
  meta.setStatic(true);
  meta.setName("meta");
  parlante.setPosition(width/2, height/2);
  parlante.setNoStroke();
  borde.setStatic(true);
  bordeA.setStatic(true);
  golpea= new FMouseJoint(parlante, width/2, height/2); //Le hago un joint al rectangulo para conectarlo al mouse
  p = new Parlante (70, 70);
  //circulo.setName("nota");
  //para agregarle bordes al mundo y los elementos no se escapen
  // mundo.setEdges();

  parlante.setRotatable(false); //para que no se gire el rectangulo cuando golpea las notas
  borde.setNoStroke();
  bordeA.setNoStroke();
  meta.setNoFill();
  parlante.setNoFill();
  //parlante.attachImage(radio);
  //cancion.play();
}
...
boolean eliminarNota=false;
  ...
  eliminarNota=false;}
    ...
    image(efectoRadio[cambiaEfecto], 155, 100);
